...
The canonical core request carries ``backend_id: str | None`` as of the published
omnibase-core 0.46.8, so this test drives the real base wire model directly.  It
previously used a temporary ``_PinnedDelegationRequest`` subclass to stay
testable against the pre-release core pin; that shim is gone, because a subclass
seam would keep passing even if the field were dropped from the canonical model.
...
def _request(*, backend_id: str | None) -> ModelDelegationRequest:
