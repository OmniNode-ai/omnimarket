...
class TestCanonicalCloudTargetCapability:
    ...
    def test_vertex_routing_tier_use_for_excludes_code_generation(self) -> None:
        ...
        OMN-16833 removed the ``vertex-gemini-flash`` entry from every tier — no
        lane can bind ``cloud-vertex-gemini`` (endpoint_url null, the render path
        does not read the env var, and the dev lane's ADC token file is
        /dev/null), so the rung was declared and unreachable at once. The backend
        DECLARATION stays parked for the eventual ADC mount.
        ...
        for entry in vertex_entries:
            assert "code_generation" not in entry["use_for"], (
                "cloud-vertex-gemini must never carry code_generation in a "
                "tier's use_for — OMN-13140 keeps exactly one canonical Gemini "
                "target on the escalation path"
            )
