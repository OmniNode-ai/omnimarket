...
def _install_ladder(
    ...
    *,
    ...
    # declares exactly ONE backend per tier (``_TIER_BACKEND_ID``), so the
    # truthful answer here is always "no sibling" — without this double the call
    # would reach the real authority and read the live ``routing_tiers.yaml``
    # instead of this fixture, which is what these tests exist to avoid.
    monkeypatch.setattr(
        port_mod,
