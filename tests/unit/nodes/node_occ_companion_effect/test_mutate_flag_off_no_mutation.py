...
mutate write path and (2) retried then fail-closed on a persistent API error
(OMN-16071 — `ci:ready` is CI-gating, not observability, so a label POST
failure must abort authoring rather than report a companion as successfully
authored while it silently carries only the marker).
"""
...
@pytest.mark.unit
def test_marker_label_failure_retries_then_fails_authoring(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    # OMN-16071: `ci:ready` is CI-gating, not observability — a persistent label
    # API failure must retry (bounded, via call_with_retry) and then propagate,
    # never silently report a companion as successfully authored while it
    # carries only the marker and can never pass CI Summary.
    calls: list[str] = []

    def _raise(method: str, path: str, **k: object) -> object:
        calls.append(path)
        raise GitHubApiError("boom")  # status_code=None -> classified transient
    ...
    # call_with_retry sleeps between attempts; skip the real delay in a unit test.
    monkeypatch.setattr("omnimarket.occ_git_transport.time.sleep", lambda _: None)
    handler = HandlerOccCompanionEffect()
    with pytest.raises(GitHubApiError, match="boom"):
        handler._apply_machine_minted_label(
            "OmniNode-ai", "onex_change_control", 4284, "t"
        )
    assert len(calls) == 3  # _RETRY_MAX_ATTEMPTS, all exhausted
    assert all("/labels" in path for path in calls)
