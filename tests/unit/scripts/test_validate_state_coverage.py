...
@pytest.mark.unit
def test_topic_marker_only_node_diff_does_not_promote_strict_coverage(
    scc_module: object, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Adding only onex-topic markers to node test fixtures is not a semantic
    state-coverage change and must not promote baselined node gaps to FAIL."""
    import subprocess
    import types

    node_name = "node_chain_diff"
    changed_file = f"src/omnimarket/nodes/{node_name}/tests/test_handler_chain_diff.py"
    name_only = f"{changed_file}\n"
    marker_only_diff = f"""diff --git a/{changed_file} b/{changed_file}
--- a/{changed_file}
+++ b/{changed_file}
@@ -20 +20 @@
-_TOPIC = "onex.evt.omnimarket.chain-diff-completed.v1"
+_TOPIC = "onex.evt.omnimarket.chain-diff-completed.v1"  # onex-topic-test-fixture
"""

    def _fake_run(args: object, *_a: object, **_kw: object) -> types.SimpleNamespace:
        argv = list(args)  # type: ignore[arg-type]
        if "--unified=0" in argv:
            return types.SimpleNamespace(
                returncode=0, stdout=marker_only_diff, stderr=""
            )
        return types.SimpleNamespace(returncode=0, stdout=name_only, stderr="")

    monkeypatch.setattr(subprocess, "run", _fake_run)
    _nodes, directly_modified = scc_module._get_changed_nodes("origin/dev")  # type: ignore[attr-defined]
    assert node_name not in directly_modified


@pytest.mark.unit
def test_non_marker_node_diff_still_promotes_strict_coverage(
    scc_module: object, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Real code/test edits still mark the node strict-eligible."""
    import subprocess
    import types
    ...
    changed_file = f"src/omnimarket/nodes/{node_name}/tests/test_handler_chain_diff.py"
    name_only = f"{changed_file}\n"
    semantic_diff = f"""diff --git a/{changed_file} b/{changed_file}
--- a/{changed_file}
+++ b/{changed_file}
@@ -20 +20 @@
-expected = "old"
...
"""
    ...
    def _fake_run(args: object, *_a: object, **_kw: object) -> types.SimpleNamespace:
        argv = list(args)  # type: ignore[arg-type]
        if "--unified=0" in argv:
            return types.SimpleNamespace(returncode=0, stdout=semantic_diff, stderr="")
        return types.SimpleNamespace(returncode=0, stdout=name_only, stderr="")
