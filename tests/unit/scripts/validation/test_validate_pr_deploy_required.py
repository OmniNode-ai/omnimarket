...
"""Regression tests for validate_pr_deploy_required.py (OMN-9685 propagation to omnimarket).
...
class TestFindRuntimePaths:
    ...
    def test_evidence_collector_matched_as_node_service(self) -> None:
        """Node-internal service files under src/*/nodes/**/ are treated as runtime paths.
        The old catch-all src/omnimarket/**/*.py also matched; the narrowed matcher still
        catches this via src/*/nodes/**/*.py, which is correct — node services CAN affect runtime."""
        files = [
            "src/omnimarket/nodes/node_dod_verify/services/evidence_collector.py",
        ]
        hits = find_runtime_paths(files)
        assert len(hits) == 1

    def test_node_handler_matched(self) -> None:
        files = ["src/omnimarket/nodes/node_dod_verify/handlers/handler_dod_verify.py"]
        hits = find_runtime_paths(files)
        assert len(hits) == 1
        assert hits[0] == files[0]

    def test_omnimarket_node_handler_matched(self) -> None:
        files = ["src/omnimarket/nodes/node_merge_sweep/handlers/handler_merge.py"]
        hits = find_runtime_paths(files)
        assert len(hits) == 1
    ...
    def test_root_dockerfile_matched(self) -> None:
        assert find_runtime_paths(["Dockerfile"]) != []

    def test_contract_yaml_in_src_matched(self) -> None:
        files = ["src/omnimarket/nodes/node_x/contract.yaml"]
        hits = find_runtime_paths(files)
        assert len(hits) == 1

    def test_cli_file_matched(self) -> None:
        files = ["src/omnimarket/cli/cli_commands.py"]
        hits = find_runtime_paths(files)
        assert len(hits) == 1
    ...
    def test_top_level_node_module_matched(self) -> None:
        files = ["src/omnimarket/nodes/node_contract_resolve_compute.py"]
        hits = find_runtime_paths(files)
        assert len(hits) == 1
...
def _write_contract(contracts_dir: Path, ticket_id: str, has_deploy: bool) -> None:
    ...
    if has_deploy:
        data["dod_evidence"] = [
            {"checks": [{"check_value": "deploy omnimarket to .201"}]}
        ]
...
class TestValidatePrDeployGate:
    ...
    def test_evidence_collector_requires_deploy_evidence(self, tmp_path: Path) -> None:
        """Node-internal service files are runtime paths — they require deploy evidence.
        PR #407 incident: skip-token was the wrong fix; real fix is proper dod_evidence."""
        result = validate_pr_deploy_gate(
            changed_files=[
                "src/omnimarket/nodes/node_dod_verify/services/evidence_collector.py",
            ],
            pr_body="Closes OMN-9720",
            contracts_dir=tmp_path,
        )
    ...
    def test_test_file_gate_skipped(self, tmp_path: Path) -> None:
        result = validate_pr_deploy_gate(
            changed_files=["tests/unit/nodes/node_dod_verify/test_adversarial.py"],
            pr_body="Closes OMN-9720",
            contracts_dir=tmp_path,
        )
        assert result.passed
    ...
    def test_skip_token_in_body_does_not_bypass(self, tmp_path: Path) -> None:
        """skip-deploy-gate token must NOT bypass the gate (OMN-9685)."""
        result = validate_pr_deploy_gate(
            changed_files=["src/omnimarket/nodes/node_x/handlers/handler_x.py"],
