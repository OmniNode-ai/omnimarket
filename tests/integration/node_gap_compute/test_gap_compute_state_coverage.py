...
"""Contract-declared state-coverage regression tests for node_gap_compute.
...
def test_gap_compute_declares_output_topics() -> None:
    ...
    assert "onex.evt.omnimarket.gap-compute-completed.v1" in publish_topics


def test_gap_compute_declares_output_fields() -> None:
    """Every contract-declared output-class field keeps its declared name."""
    outputs = _load_contract()["outputs"]
    assert "best_effort_findings" in outputs
    assert "report_path" in outputs
    assert "run_id" in outputs
    assert "scope" in outputs
    assert "subcommand" in outputs
