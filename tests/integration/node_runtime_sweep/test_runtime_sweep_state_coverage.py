...
"""Contract-declared state-coverage regression tests for node_runtime_sweep.

OMN-13674 / OMN-13676 (WS-5 Wave 2) under the strengthened full
...
def test_runtime_sweep_declares_output_topics() -> None:
    ...
    assert "onex.evt.omnimarket.runtime-sweep-finding.v1" in publish_topics
    assert "onex.evt.omnimarket.workflow-stalled.v1" in publish_topics
    assert "onex.evt.omnimarket.workflow-timeout.v1" in publish_topics


def test_runtime_sweep_declares_output_fields() -> None:
