...
"""Contract-declared state-coverage regression tests for node_baseline_compare.
...
def test_baseline_compare_declares_output_topics() -> None:
    ...
    assert "onex.evt.omnimarket.baseline-compared.v1" in publish_topics


def test_baseline_compare_declares_output_fields() -> None:
    """Every contract-declared output-class field keeps its declared name."""
    outputs = _load_contract()["outputs"]
    assert "compared_at" in outputs
