...
2. The shape `contracts/<TICKET-ID>` — catches scripts that `cd` into a repo
...
| Category                                                                 | Count |
|--------------------------------------------------------------------------|------:|
| References inside a repo's *own* `contracts/<TICKET-ID>.yaml` work-tracking files (other repos: omnibase_compat, omnibase_infra, etc.) — those are self-references in their own dod_evidence; out of scope for this PR | ~600 |
...
| `tests/unit/nodes/node_dod_verify/test_durable_evidence_gate.py` — uses a deliberate **fake** `contract_rel_path` argument; the gate's `load_contract_on_ref` is stubbed and never reads disk; refers to OCC's `<occ>/contracts/`, not omnimarket's | 9 |
...
84 work-tracking YAML files are pure dod_evidence artifacts.
...
- 84 work-tracking YAML files at `omnimarket/docs/work-tracking/contracts/`.
