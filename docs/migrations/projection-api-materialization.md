...
Before OMN-13150 `node_projection_routing_decision` targeted `omnibase_infra`.
OMN-13150 repointed it to `omnidash_analytics` (the correct consumer), which
...
OMN-13122 originally pointed `node_projection_routing_decision` to
`omnibase_infra`; OMN-13150 (FIX-1) repointed to `omnidash_analytics` and
added runtime proof. This is the canonical example of an incorrect DB target
being caught and corrected post-merge.
