...
"""Shared OCC evidence event models — canonical OWNER surface (OMN-13209 / A2).

This module is the OWNER of the OCC evidence draft DTOs (it defines them here,
rather than re-exporting them from a sibling node package). The deployment/OCC
nodes (`node_occ_evidence_draft_orchestrator`,
`node_occ_evidence_validator_compute`) import the OCC models and the runtime-lane
/ deployment-proof DTOs from this single shared event surface so they never reach
into a sibling node's private model package.

Runtime-deployment primitives (`EnumRuntimeLane`, `ModelRuntimeDeploymentProof`)
are re-exported from their canonical `omnibase_core` home (graduated from
omnibase_compat in OMN-13209); this surface keeps a stable import path for the
OCC nodes.
...
decides acceptance. The PASS path publishes the existing
``omnibase_compat ... ModelEvidenceValidationResult`` on
``onex.evt.omnimarket.evidence-validated.v1`` so the OCC PR writer fires;
``ModelOccEvidenceDraftValidationResult`` here is the INTERNAL reject/audit shape
ONLY — it is never published on ``evidence-validated.v1``.
"""
...
class EnumEvidenceLifecycleState(StrEnum):
    ...
    Values mirror the compat ``EvidenceLifecycleState`` literal
    (``omnibase_compat.contracts.evidence_pipeline.wire.types``). A model-drafted
