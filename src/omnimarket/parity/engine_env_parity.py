"""Pure env-parity evaluation engine shared by compute and collect nodes.

Extracted from ``HandlerEnvParityCompute`` in OMN-13925 so the live collection
front-end (``node_env_parity_collect_effect``) can evaluate parity over
freshly-collected lane snapshots without importing another node's private
handler package. The engine itself performs no I/O: it takes a typed request
plus a typed contract config and returns a typed result.
"""
...
def evaluate_env_parity(
    ...
    for rule in variables:
        ...
        if unknown_rule_lanes:
            return env_parity_error_result(
                request,
...
def env_parity_error_result(
    request: ModelEnvParityComputeRequest, message: str
) -> ModelEnvParityComputeResult:
    """Build the typed fail-fast error result for an env-parity request."""
    return ModelEnvParityComputeResult(
        status="error",
        parity_ok=False,
        scope=request.scope,
...
__all__ = ["env_parity_error_result", "evaluate_env_parity"]
