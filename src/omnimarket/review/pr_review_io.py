...
"""Shared PR-review node-I/O models for the pr_review node family (OMN-13212 / B2).

OWNER module for the request/result/state models exchanged between the canonical
PR-review nodes rebuilt from the deleted ``node_pr_review_bot`` ``workflow``
shell:

  - node_pr_review_orchestrator   (ORCHESTRATOR)
  - node_pr_review_fsm_reducer     (REDUCER)
  - node_github_review_effect      (EFFECT)
...
The ``ReviewVerdict`` output shape and the ``EnumFsmPhase`` / ``EnumThreadStatus``
/ ``EnumPrVerdict`` vocabularies are preserved verbatim from the legacy
``node_pr_review_bot.models.models`` module so that ``skill_mapping`` /
``result_model`` contracts and replay of historical chains stay valid.
"""
...
class PrReviewFindingEvidence(BaseModel):
    """Evidence model for PR review findings, extending the base with line anchors.

    The GitHub review EFFECT uses line_start / line_end to anchor review threads
    to specific diff lines. The shared ModelFindingEvidence does not carry these
    fields, so a PR-specific evidence model is defined here.
    """
...
# ---------------------------------------------------------------------------
# Input: ReviewRequest (the pr-review-bot-start command payload)
# ---------------------------------------------------------------------------


class ReviewRequest(BaseModel):
    """Command payload to start a PR review run."""
...
# ---------------------------------------------------------------------------
# Review finding (PR extension of the shared ModelReviewFinding)
# ---------------------------------------------------------------------------


class ReviewFinding(BaseModel):
    """A single finding from a reviewer model, enriched with PR-specific context."""
...
# ---------------------------------------------------------------------------
# Verdict (the pr-review-bot-completed terminal event / result_model)
# ---------------------------------------------------------------------------
...
class ReviewVerdict(BaseModel):
    """Aggregated result for a complete PR review run.

    Output shape preserved verbatim from the legacy node so the
    ``skill_mapping`` ``result_model`` and replay of historical
    ``pr-review-bot-completed`` chains stay valid.
    """
