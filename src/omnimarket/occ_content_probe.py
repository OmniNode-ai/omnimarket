...
def extract_symbol_candidates(
    ...
    PR, or a Python PR that only edits function bodies, yields zero candidates
    and therefore no content-bound check. Widening the grammar (added non-comment
    source lines via ``grep -F``, added-file 404 probes) is deliberate follow-up,
    which is why the content-bound binding ships default-OFF.
    """
