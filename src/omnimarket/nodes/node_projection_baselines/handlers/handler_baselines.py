...
class BaselinesProjectionRunner(BaseProjectionRunner):
    ...
    async def project_event(
        ...
        computed_at_utc = safe_parse_date(
            data.get("computed_at_utc") or data.get("computedAtUtc") or data.get("computed_at")
        )
        window_start_utc = (
        ...
        raw_comparisons_all = data.get("comparisons") if isinstance(data.get("comparisons"), list) else []
        if len(raw_comparisons_all) > MAX_BATCH_ROWS:
            logger.warning(
                "baselines snapshot %s: %d comparison rows, capping at %d",
                snapshot_id, len(raw_comparisons_all), MAX_BATCH_ROWS,
            )
        raw_comparisons = raw_comparisons_all[:MAX_BATCH_ROWS]

        raw_trend_all = data.get("trend") if isinstance(data.get("trend"), list) else []
        if len(raw_trend_all) > MAX_BATCH_ROWS:
            logger.warning(
                "baselines snapshot %s: %d trend rows, capping at %d",
                snapshot_id, len(raw_trend_all), MAX_BATCH_ROWS,
            )
        raw_trend = raw_trend_all[:MAX_BATCH_ROWS]

        raw_breakdown_all = data.get("breakdown") if isinstance(data.get("breakdown"), list) else []
        if len(raw_breakdown_all) > MAX_BATCH_ROWS:
            logger.warning(
                "baselines snapshot %s: %d breakdown rows, capping at %d",
                snapshot_id, len(raw_breakdown_all), MAX_BATCH_ROWS,
            )
        ...
        for t in raw_trend:
            ...
            trend_by_date[date_val] = {
                ...
                "date": date_val,
                "avg_cost_savings": str(max(0.0, min(99.0, _safe_float(
                    t.get("avg_cost_savings") or t.get("avgCostSavings")
                )))),
                "avg_outcome_improvement": str(max(0.0, min(99.0, _safe_float(
                    t.get("avg_outcome_improvement") or t.get("avgOutcomeImprovement")
                )))),
        ...
        for c in raw_comparisons:
            ...
            if not pattern_id:
                logger.warning("Skipping comparison row with blank pattern_id for snapshot %s", snapshot_id)
                continue
            ...
            comparison_rows.append({
                ...
                "pattern_name": str(c.get("pattern_name") or c.get("patternName") or ""),
                "sample_size": _safe_int(c.get("sample_size") or c.get("sampleSize")),
                "window_start": str(c.get("window_start") or c.get("windowStart") or ""),
                "window_end": str(c.get("window_end") or c.get("windowEnd") or ""),
                "token_delta": json.dumps(c.get("token_delta") or c.get("tokenDelta") or {}),
                "time_delta": json.dumps(c.get("time_delta") or c.get("timeDelta") or {}),
                "retry_delta": json.dumps(c.get("retry_delta") or c.get("retryDelta") or {}),
                "test_pass_rate_delta": json.dumps(c.get("test_pass_rate_delta") or c.get("testPassRateDelta") or {}),
                "review_iteration_delta": json.dumps(c.get("review_iteration_delta") or c.get("reviewIterationDelta") or {}),
        ...
        for b in raw_breakdown:
            ...
            breakdown_by_action[action] = {
                ...
                "avg_confidence": str(max(0.0, min(1.0, _safe_float(
        ...
        for row in comparison_rows:
            queries.append((
                ...
                """,
                (
                    row["snapshot_id"], row["pattern_id"], row["pattern_name"], row["sample_size"],
                    row["window_start"], row["window_end"], row["token_delta"], row["time_delta"],
                    row["retry_delta"], row["test_pass_rate_delta"], row["review_iteration_delta"],
                    row["recommendation"], row["confidence"], row["rationale"],
                ),
            ))
        ...
        for row in trend_by_date.values():
            queries.append((
                ...
                """,
                (
                    row["snapshot_id"], row["date"], row["avg_cost_savings"],
                    row["avg_outcome_improvement"], row["comparisons_evaluated"],
                ),
            ))
        ...
        for row in breakdown_by_action.values():
            queries.append((
                ...
                """,
                (row["snapshot_id"], row["action"], row["count"], row["avg_confidence"]),
            ))
