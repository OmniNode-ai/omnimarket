...
class ModelPrLifecycleFixResult(BaseModel):
    ...
    block_reason: EnumPrBlockReason = Field(..., description="Block reason that was routed.")
    fix_applied: bool = Field(..., description="Whether a fix action was dispatched.")
    fix_action: str = Field(..., description="Fix action taken or would be taken (dry_run).")
