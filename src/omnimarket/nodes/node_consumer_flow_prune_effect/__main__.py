...
"""Operator entry point for the consumer-flow prune node.

    uv run python -m omnimarket.nodes.node_consumer_flow_prune_effect \\
        (--dry-run | --archive-dir DIR | --s3-uri s3://BUCKET/PREFIX/ --kms-key KEY) \\
        --report-dir DIR [--retention-days N] [--as-of ISO8601] [--max-days N] \\
        [--aws-profile P]

The database is the one the contract's dsn_env names (OMNINODE_INTERNAL_DB_URL);
...
_OK = {
    EnumConsumerFlowPruneVerdict.PRUNED,
    EnumConsumerFlowPruneVerdict.DRY_RUN,
    EnumConsumerFlowPruneVerdict.NOTHING_TO_PRUNE,
}
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(prog="node_consumer_flow_prune_effect")
    ...
    try:
        result = HandlerConsumerFlowPrune(store=store, sink=sink, cipher=cipher).handle(
            ModelConsumerFlowPruneRequest(
    ...
    (runs / f"consumer-flow-prune-{stamp}.json").write_text(
