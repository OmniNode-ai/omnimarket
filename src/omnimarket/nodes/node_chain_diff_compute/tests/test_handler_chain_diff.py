...
"""Unit tests for HandlerChainDiff (node_chain_diff_compute) — full diff scenario coverage."""
...
def _entry(
    ...
    event_type: str,
    topic: str = _TOPIC_A,
    source_node: str = "node_chain_diff_compute",
...
@pytest.mark.unit
class TestHandlerChainDiffComputeExactMatch:
...
@pytest.mark.unit
class TestHandlerChainDiffComputeMissingEvent:
...
@pytest.mark.unit
class TestHandlerChainDiffComputeUnexpectedEvent:
...
@pytest.mark.unit
class TestHandlerChainDiffComputeOrderMismatch:
...
@pytest.mark.unit
class TestHandlerChainDiffComputeTopicMismatch:
...
@pytest.mark.unit
class TestHandlerChainDiffComputeDeterminism:
    ...
    def test_diff_chains_fn_matches_handler(self) -> None:
        chain = (_entry(1, "EventA"), _entry(2, "EventB"))
        handler_result = HandlerChainDiff().handle(
            ModelChainDiffRequest(expected=chain, observed=chain)
        )
        fn_result = diff_chains(chain, chain)
        assert handler_result == fn_result
