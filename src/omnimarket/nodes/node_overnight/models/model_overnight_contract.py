...
"""Wire types for the overnight session contract.

Lives here rather than in ``omnibase_core`` because ``node_overnight`` is the
only consumer in the org, and it is not interchangeable with core's
``ModelSessionContract`` family despite the close resemblance. Core's
``ModelSessionPhaseSpec.required_outcomes`` is
``tuple[EnumCompletionOutcome, ...]`` — the closed set success/failure/partial/
timeout/cancelled/skipped — whereas these phases carry domain outcome names such
as ``delegation_pipeline_active`` that the pipeline resolves by name. Re-pointing
at core would not fail loudly; it would reject every real contract this node
loads. The two models answer different questions and only share a field name.

Previously imported from ``onex_change_control``. Product code no longer depends
on the governance repo (operator ruling), and a type with exactly one consumer
does not belong in core either, so it lives with its consumer.
