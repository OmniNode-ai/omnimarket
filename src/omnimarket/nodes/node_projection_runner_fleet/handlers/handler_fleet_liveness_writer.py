...
# Scoped to the OBSERVING host. A second observing host's runners are not this
# observation's to tombstone, and a query that ignored the host would have each
# host delete the other's fleet on every cycle.
# Scoped on observing_host, NOT on host: the observation is one observer's
# view of the pool, and a runner that moved between physical hosts must not
# look deregistered to the observer that still sees it.
_SELECT_KNOWN_FOR_HOST = f"""
    SELECT runner_name FROM {TABLE_FLEET} WHERE observing_host = $1
"""

_DELETE_RUNNER = f"""
    DELETE FROM {TABLE_FLEET} WHERE runner_name = $1 AND observing_host = $2
"""
...
class FleetLivenessProjectionWriter(BaseProjectionRunner):
    ...
    async def _project_observation(
        ...
        known_rows = await self.db.execute(_SELECT_KNOWN_FOR_HOST, observation.host)
        ...
        for name in result.tombstoned_runner_names:
            await self.db.execute(_DELETE_RUNNER, name, observation.host)
